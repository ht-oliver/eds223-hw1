---
title: "Environmental Justice in Wayne County, MI"
format: html
warning: False
execute: 
  warning: False
...
## Purpose:
The purpose of this document is to practice geospatial data analysis by creating visually appealing maps that display an environmental injustice in Michigan's Wayne County.
...
### 2)  Read in geodatabase of EJScreen data at the Census Block Group level<sup/>1<sup>

```{r}
#| message: False
ejscreen <- sf::st_read(here::here("data", "ejscreen","EJSCREEN_2023_BG_StatePct_with_AS_CNMI_GU_VI.gdb"), quiet = TRUE) %>% 
  select(PTSDF, STATE_NAME, ST_ABBREV, CNTY_NAME, ACSTOTPOP, LOWINCOME, LOWINCPCT, Shape)
```

### 3)  Read in other data to be used in map display<sup/>2, 3<sup>
...
# Create extent border in inset map from study area
